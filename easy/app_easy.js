...
http.createServer((req, res) => {
	...
	switch (req.url) {
		// Home
		case '/':
			let home = 'Bienvenidas/os a DH MOVIES el mejor sitio para encontrar las mejores películas,\nIncluso mucho mejor que Netflix, Cuevana y PopCorn!!!.\n\n'
			...
			home += '='.repeat(80) + '\n';
			home += ' '.repeat(29) + 'PELICULAS EN CARTELERA\n'
			home += '='.repeat(80) + '\n\n';
			home += `Total de películas en cartelera: ${movies.length}\n\n`
		...
		// En cartelera
		case '/en-cartelera':
			let cartelera = '='.repeat(80) + '\n';
			cartelera += ' '.repeat(29) + 'EN CARTELERA\n'
			cartelera += '='.repeat(80) + '\n\n';
			cartelera += `Total de películas en cartelera: ${movies.length}\n\n`

			for (let movie of movies) {
				cartelera += `Título: ${movie.title}\nReseña: ${movie.overview}\n\n`;
			}
		...
		// Sucursales
		case '/sucursales':
			let sucursales = '='.repeat(80) + '\n';
			sucursales += ' '.repeat(29) + 'NUESTRAS SALAS\n'
			sucursales += '='.repeat(80) + '\n\n';

			sucursales += `Total de salas: ${theaters.length}\n\n`

			for (let theater of theaters) {
				sucursales += `Nombre: ${theater.name}\nDirección: ${theater.address}\nDescripció: ${theater.description}\n\n`;
			}
			sucursales += '\n' + '='.repeat(80) + '\n';
		...
		case '/preguntas-frecuentes':
			let faq = '='.repeat(80) + '\n';
			faq += ' '.repeat(29) + 'PREGUNTAS FRECUENTES\n'
			faq += '='.repeat(80) + '\n\n';
			faq += `Total de preguntas: ${faqs.length}\n\n`
			for (let faq of faqs) {
				faq += `Pregunta: ${faq.faq_title}\nRespuesta: ${faq.faq_answer}\n\n`;
			}
			faq += '\n' + '='.repeat(80) + '\n';
			faq += '\nRecordá que podés visitar las secciones:\n\n• Inicio (/)\n• En Cartelera (/en-cartelera)\n• Más Votadas (/mas-votadas)\n• Sucursales (/sucursales)\n• Contacto (/contacto)\n• Preguntas Frecuentes (/preguntas-frecuentes)';
			faq += '\n\n' + '='.repeat(80) + '\n';
		...
		// Default
		default:
			let notFound = '='.repeat(80) + '\n';
			notFound += ' '.repeat(29) + 'PÁGINA NO ENCONTRADA\n'
			notFound += '='.repeat(80) + '\n\n';
			res.end(notFound);
	}
